...
public class GameEntryBozukoActivity extends BozukoControllerActivity implements
	...
	public void progressRunnableError() {
		...
		if (errorType.compareTo("facebook/auth") == 0) {
			makeDialog(errorMessage, errorTitle,
	...
	public void setupView() {
		...
		if (listview.getAdapter() == null) {
			...
			if (game.prizes.size() > 0 && game.consoldationPrizes.size() > 0) {
				mergeAdapter.addView(getSpacer(), false);
				mergeAdapter.addView(getGroupTitleView("Available Prizes"),
						false);
				...
				mergeAdapter.addAdapter(new PrizeListAdapter(
						game.consoldationPrizes));
			}

			mergeAdapter.addView(getSpacer(), false);
			mergeAdapter.addView(
					getRulesCellView(game.requestInfo("rules"),
							R.drawable.cellbtn), false);
	...
	public void setupButton() {
		((Button) findViewById(R.id.entergame)).setText(game.gameState
				.requestInfo("button_text"));
		((Button) findViewById(R.id.entergame)).setOnClickListener(this);

		if (game.requestInfo("type").compareTo("scratch") == 0) {
			try {
				GameResult result = new GameResult(
						game.gameState.requestInfo("game_id"));
				User usr = new User("1");
				usr.getObject("1", BozukoDataBaseHelper.getSharedInstance(this));
				result.getObject(game.gameState.requestInfo("game_id"),
						BozukoDataBaseHelper.getSharedInstance(this),
						usr.requestInfo("bozukoid"));
				PrizeObject prize = null;
				try {
					prize = new PrizeObject(
							game.gameState.requestInfo("game_id"));
					prize.getObject(game.gameState.requestInfo("game_id"),
							BozukoDataBaseHelper.getSharedInstance(this));
				} catch (Throwable t) {

				}
				if (result.checkInfo("scratchBitmap")
						&& !result.checkInfo("isFinished")) {
					...
					if (result.checkInfo("bozukoid")) {
						result.delete(game.gameState.requestInfo("game_id"),
								BozukoDataBaseHelper.getSharedInstance(this));
						prize.delete(game.gameState.requestInfo("game_id"),
								BozukoDataBaseHelper.getSharedInstance(this));
					}
					loadButton();
				}
	...
	public void loadButton() {
		if (game.gameState.requestInfo("button_enabled").compareTo("true") != 0) {
			...
			findViewById(R.id.agree).setVisibility(View.INVISIBLE);
			findViewById(R.id.nextgame).setVisibility(View.VISIBLE);
			((TextView) findViewById(R.id.nextgame)).setText(game.gameState
					.requestInfo("button_text"));
		} else {
			findViewById(R.id.entergame).setVisibility(View.VISIBLE);
			findViewById(R.id.entergame).setEnabled(true);
	...
	public void getGameState() {
		...
		try {
			...
			String url = GlobalConstants.BASE_URL
					+ game.gameState.requestInfo("linksgame_state");
			url += "?token=" + mprefs.getString("token", "")
					+ "&mobile_version=" + GlobalConstants.MOBILE_VERSION;
			url += "&accuracy=" + mprefs.getString("acc", "0");
			HttpRequest req = new HttpRequest(new URL(url
					+ String.format("&ll=%s,%s",
							mprefs.getString("clat", "0.00"),
							mprefs.getString("clon", "0.00"))));
		...
		if (mprefs.getBoolean("facebook_login", false)) {
			if (game.requestInfo("type").compareTo("scratch") == 0) {
				Intent intent = new Intent(this,
						ScratchGameBozukoActivity.class);
				startActivity(intent);
